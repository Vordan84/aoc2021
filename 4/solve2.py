...
# main
# goal: find last board to win and returns its score
#----------------------------------------------
...
with filePath.open('r') as input:
            ...
            while not searchDone:
                try:
                    ...
                    if -1 != bingoBoardIndex:
                        if -1 == winningBoards[bingoBoardIndex]:    
                            numBoardsNotWonYet = 0
                            
                            for lastDrawIndex in winningBoards:
                                if -1 == lastDrawIndex:
                                    numBoardsNotWonYet += 1
                            
                            winningBoards[bingoBoardIndex] = bingoDrawIndex
                            
                            if 1 == numBoardsNotWonYet:                        
                                lastWinningBoardIndex = bingoBoardIndex
                                break
    ...
    if -1 == lastWinningBoardIndex:
        lastWinningDrawIndex = 0
        for boardIndex, drawIndex in enumerate(winningBoards):
            if lastWinningDrawIndex < drawIndex:
                lastWinningDrawIndex = drawIndex
                lastWinningBoardIndex = boardIndex            
    
    score = computeBoardScore(boardNumbers, lastWinningBoardIndex, draws, winningBoards[lastWinningBoardIndex])
    print(score)
